...
**Known open bug**: new-account signup intermittently fails to auto-create its circle/profile — the insert is rejected by RLS as if unauthenticated, even though sign-up/sign-in itself succeeds at the Supabase Auth level. Not yet root-caused (see issue 15's "Post-launch amendments"). Only the one grandfathered account (`nz.eason.chen@gmail.com`) is in use right now, so this hasn't blocked anything yet, but fix it before letting anyone else sign up.
