...
- **一个还没查出根本原因的已知问题**：新注册一个全新账号，偶尔会出现自动创建圈子/档案失败的情况——插入请求被 RLS 拒绝，表现得就像完全没登录一样，但注册/登录本身在 Supabase Auth 这一层是成功的（从 Supabase 自己的请求日志能看到：`POST /auth/v1/token` 立刻返回 200，紧接着 `POST /rest/v1/circles` 却返回 403）。在两个不同的浏览器/设备上都复现了，所以大概率不是某一个浏览器的存储/隐私设置问题——还需要真正定位并修复。在向这个已经"祖父迁移"过的唯一账号之外的任何人开放注册之前，需要先跟进这个问题。
