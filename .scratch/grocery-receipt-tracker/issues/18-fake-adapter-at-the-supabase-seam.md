...
- ~~`src/lib/alerts.ts` still falls back to `""` rather than the Source Text when a Product has no Translation.~~ **Closed**: `fetchAlerts` now falls back to `canonical_name_en`, matching `productDetail.ts` and `exportCsv.ts`, with a regression test. Two sites in `src/lib/monthlyReport.ts` break the same rule and are **still open**: `nameZh` falls back to `""` (line ~127), and `fetchProductNames` (line ~170) casts the rows to `canonical_name_zh: string`, which the nullable column contradicts — a Supabase-boundary `as` in production code that the `as never`/`as unknown as` sweep missed.
...
- ~~`src/lib/alerts.ts` 在 Product 没有 Translation 时仍然兜底成 `""` 而非 Source Text。~~ **已关闭**：`fetchAlerts` 现已兜底到 `canonical_name_en`，与 `productDetail.ts`、`exportCsv.ts` 一致，并补了回归测试。`src/lib/monthlyReport.ts` 中还有两处违反同一规则，**仍未关闭**：`nameZh` 兜底成 `""`（约第 127 行）；`fetchProductNames`（约第 170 行）把行强转为 `canonical_name_zh: string`，与该列可空的事实矛盾——这是一处生产代码里的 Supabase 边界 `as`，此前只 grep `as never` / `as unknown as` 时漏掉了。
