...
- **Known open issue, not yet root-caused**: signing up a brand-new account intermittently fails to auto-create its circle/profile — the insert gets rejected by RLS as if the request weren't authenticated at all, even though the sign-up/sign-in itself succeeds at the Supabase Auth level (confirmed via Supabase's own request logs: `POST /auth/v1/token` returns 200 immediately followed by `POST /rest/v1/circles` returning 403). Reproduced across two different browsers/devices, so likely not one browser's storage/privacy settings — still needs a real fix. Follow up before opening signup to anyone beyond the single grandfathered account.
